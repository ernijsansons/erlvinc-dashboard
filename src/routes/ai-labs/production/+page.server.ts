...
export const load: PageServerLoad = async ({ platform }) => {
  try {
    ...
    if (dev) {
      ...
      res = await fetch("http://127.0.0.1:8787/api/planning/runs?limit=100&status=completed");
    } else if (platform?.env?.GATEWAY) {
      // Production: use service binding
      res = await platform.env.GATEWAY.fetch("https://_/api/planning/runs?limit=100&status=completed");
    } else {
    ...
    if (!res.ok) {
      ...
      console.error("Failed to fetch production runs:", res.status, text);
