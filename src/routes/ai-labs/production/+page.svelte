<script lang="ts">
	...
	const taskByRunId = $derived(Object.fromEntries(data.tasks.map((t) => [t.run_id, t])));
...
{:else}
	{#if data.runs.length === 0}
		<p class="empty-hint-banner">
			Complete research runs to add validated ideas to the production pipeline.
		</p>
	{/if}
			...
			{#each data.runs as run}
				{#if !taskByRunId[run.id]}
					<div class="card-action-row">
						...
						<button type="button" class="btn-assign" onclick={() => openAssignModal(run.id)}>
							Assign to Naomi
						</button>
					</div>
				{/if}
