...
export function generateRalphLoopJSON(
	...
	const _customerIntel = artifacts['customer-intel']?.content || {};
	const _marketResearch = artifacts['market-research']?.content || {};
	const _competitiveIntel = artifacts['competitive-intel']?.content || {};
	...
	const _contentEngine = artifacts['content-engine']?.content || {};
...
function _extractArray(obj: Record<string, unknown>, key: string): string[] | undefined {
	const value = obj[key];
		...
		appPages.forEach((page: unknown, i: number) => {
			if (typeof page === 'object' && page !== null) {
				...
				features.push({
					id: `feature-page-${i}`,
					name:
						extractString(pageObj, 'title') || extractString(pageObj, 'name') || `Page ${i + 1}`,
					description:
						extractString(pageObj, 'description') || extractString(pageObj, 'purpose') || '',
		...
		apiRoutes.forEach((route: unknown) => {
			if (typeof route === 'object' && route !== null) {
				...
				endpoints.push({
					...
					method:
						(extractString(routeObj, 'method') as 'GET' | 'POST' | 'PUT' | 'DELETE' | 'PATCH') ||
						'GET',
			...
			if (typeof m === 'object' && m !== null) {
				...
				if (name && !metrics.some((existing) => existing.name === name)) {
					metrics.push({
						name,
						description:
							extractString(metric, 'description') || extractString(metric, 'target') || '',
						type: 'gauge'
					});
				}
			}
...
function determineMetricType(type: string | undefined): 'counter' | 'gauge' | 'histogram' {
	...
	if (lower.includes('gauge') || lower.includes('current') || lower.includes('rate'))
		return 'gauge';
	return 'counter';
}
	...
	if (Array.isArray(ktAssumptions)) {
		ktAssumptions.forEach((a: unknown) => {
			if (typeof a === 'object' && a !== null) {
				...
				assumptions.push({
					assumption:
						extractString(assumption, 'assumption') ||
						extractString(assumption, 'description') ||
						'',
					validation:
						extractString(assumption, 'validation') || extractString(assumption, 'test') || ''
				});
		...
		stratAssumptions.forEach((a: unknown) => {
			if (typeof a === 'object' && a !== null) {
				...
				const assumptionText =
					extractString(assumption, 'assumption') || extractString(assumption, 'description');
				// Avoid duplicates
				if (
					assumptionText &&
					!assumptions.some((existing) => existing.assumption === assumptionText)
				) {
					assumptions.push({
...
function determineSeverity(severity: string | undefined): 'critical' | 'major' | 'minor' {
	...
	if (lower.includes('critical') || lower.includes('blocking') || lower.includes('p0'))
		return 'critical';
